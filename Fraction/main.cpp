...
public:
	...
	Fraction& proper()
	{
		...
		{
			gcd = bufer_denominator % bufer_numerator;//наибольший общий делитель (алгоритм ≈вклида)
...
Fraction operator-(Fraction Left, Fraction Right) // ѕроверить вывод двух минусов ???
{
	return Fraction(Left.noproper().get_numerator() * Right.get_denominator() - Right.noproper().get_numerator() * Left.get_denominator(),
		Left.get_denominator() * Right.get_denominator()).proper();
}
